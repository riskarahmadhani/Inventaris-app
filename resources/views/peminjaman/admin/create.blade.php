...
<div class="card card-primary card-outline">
        ...
        <table class="table table-striped table-hover">
            ...
            @foreach ($items as $item)
                <tr>
                    ...
                    <td>{{ $item->quantity }}</td>
                    <td class="text-right">
                        <a href="{{ route('admin.peminjaman.update',[
                            'pegawai'=>$pegawai->id,
                            'inventari'=>$item->id,
                            'type'=>'plus'
                        ...
                        <a href="{{ route('admin.peminjaman.delete',[
                            'pegawai'=>$pegawai->id,
                            'inventari'=>$item->id,
    ...
    <form action="{{ route('admin.peminjaman.create',[
        'pegawai'=>$pegawai->id
    ]) }}" class="card-footer"
    method="POST">
    @csrf
    ...
    <div class="text-right">
        <a href="{{ route('admin.peminjaman.empty',['pegawai'=>$pegawai->id]) }}" class="btn btn-danger">
    ...
    <script>
        $(function() {
            ...
            $('#add').click(function(){
                ...
                if(id){
                    let url = "{{ route('admin.peminjaman.add',['pegawai'=>$pegawai->id, 'inventari'=>'inventaris-id']) }}";
